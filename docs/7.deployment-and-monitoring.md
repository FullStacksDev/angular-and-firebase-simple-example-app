...
> [!IMPORTANT]
>
> The base template is set up to run things locally and then deploy to a "live" environment — the one your users will use. You may be used to projects where there's at least one more environment in between (e.g. a "staging" environment). This is something we set up in the advanced example app, together with continuous deployment (CD).
>
